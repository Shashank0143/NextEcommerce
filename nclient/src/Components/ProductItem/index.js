...
const ProductItem = (props) => {
    ...
    return (
        <>
            <div className={`productItem ${props.itemView}`} onMouseEnter={() => handleMouseEnter(props?.itemView === 'recentlyView' ? props.item?.prodId : props.item?.id)}
                onMouseLeave={handleMouseLeave}>

                <div className="img_rapper">
                    <Link href={`/product/${props?.itemView === 'recentlyView' ? props.item?.prodId : props.item?.slug}`}>

                        {
                            isLoading === true ?
                                ...
                                <Image src={props.item?.images[0]} className="w-100" alt='Product Image' layout='responsive' width={0} height={0} priority="true" />
                        }
                ...
                <div className="info">
                    <Link href={`/product/${props?.itemView === 'recentlyView' ? props.item?.prodId : props.item?.slug}`}><h4 onClick={handleclick}>{props?.item?.name?.substr(0, 30) + '...'}</h4></Link>
                    ...
                    <Rating className="mt-2 mb-2" name="read-only" value={props?.item?.rating} readOnly size="small" precision={0.5} sx={{ color: "#202020" }} />

                    <div className="d-flex">
                        <span className="oldPrice">Rs {props?.item?.oldPrice}</span>
                        <span className="netPrice price-color ml-2">Rs {props?.item?.price}</span>
                    </div>
